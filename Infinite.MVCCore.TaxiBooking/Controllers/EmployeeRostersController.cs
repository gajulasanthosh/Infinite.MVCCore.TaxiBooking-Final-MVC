...
namespace Infinite.MVCCore.TaxiBooking.Controllers
{
    public class EmployeeRostersController : Controller
    {
        ...
        [HttpGet]
        [Route("Employeerosters/Search/{employeeName?}")]
        public async Task<ActionResult> Search(string employeeName)
        {
            if (employeeName != null)
            {
                List<EmployeeRosterViewModel> employeeRoster = new(); using (var client = new HttpClient())
                {
                    client.BaseAddress = new Uri(_configuration["ApiUrl:api"]);
                    var result = await client.GetAsync($"EmployeeRosters/Searchroster/{employeeName}");
                    if (result.IsSuccessStatusCode)
                    {
                        employeeRoster = await result.Content.ReadAsAsync<List<EmployeeRosterViewModel>>();
                    }
                }
                return View(employeeRoster);
            }
